...
| `--models` | 从 JSON 文件生成模型文件（默认增量模式） |
| `--full` | 配合 `--models` 使用，强制全量重新生成模型 |
| `--help` | 显示帮助信息 |
...
3. **增量模式**：检查文件修改时间，只更新需要更新的模型文件
4. **全量模式**：强制重新生成所有模型文件，忽略增量检查
5. 分析 JSON 结构，生成对应的 Dart 模型类
6. 执行 `build_runner build` 生成序列化代码
7. 格式化生成的代码
